...
#### Установка данного форка ни в коем случае не скажется на работоспособности раннее настроенных автоматизаций на базе VK Notify от uDocs, только добавит весь так недостающий функционал.
...
* **Карточки и Форматирование:** Поддержка `HTML` и `MarkdownV2` (без проблем с экранирующими слешами). Заголовки уведомлений (`title`) автоматически отделяются двойным отступом, создавая красивый эффект карточки. А новый режим `plain` позволяет отправлять "сырой" текст (удобно для логов или JSON).
* **Снэкбары и Callback-кнопки (vk_notify.answer_callback):** Кнопки, которые при нажатии генерируют скрытое событие в HA. Новая служба позволяет мгновенно остановить индикатор загрузки на кнопке и показать аккуратное всплывающее уведомление (Снэкбар) в приложении ВКонтакте.
* **Закрепление и Открепление (vk_notify.pin_message / unpin_message):** Автоматическое управление закрепами важных сообщений (например, главного пульта) в шапке беседы.
* **Медиа, Фото и Голосовые:** Нативная загрузка фотографий (`send_photo`), файлов и видео-роликов `.mp4` (`send_file`), постов на стену (`wall_post`) и аудиофайлов в виде голосовых сообщений с волной (`send_voice`).
* **Динамическое редактирование (vk_notify.edit_message):** Возможность изменять текст, медиавложения и кнопки уже отправленного сообщения в чате (идеально для дашбордов без спама в ленту).
...
* **Действие vk_notify.send_message:** Расширенный сервис для отправки сообщений с поддержкой JSON-клавиатур, каруселей-шаблонов и геометок.
...
* **Инфо о пользователях (vk_notify.get_user_info):** Служба для получения имени, фамилии и точного статуса (`online`, `last_seen`) пользователя по его ID.
* **Возврат ID (Service Response):** Службы отправки возвращают `conversation_message_id` прямо в автоматизацию, что позволяет скриптам «запоминать» ID для последующих правок.
...
* **Двусторонние события (Набор текста / Прочтение):** Home Assistant ловит события из Long Poll, когда вы только начинаете набирать текст или читаете сообщения бота.
* **Умное именование:** Интеграция автоматически формирует понятные `entity_id`, добавляя к базовому имени название чата и его `peer_id`.
* **Ручной ввод peer_id (Обход скрытых бесед):** Полноценная поддержка новых и неактивных бесед! На этапе добавления можно не только выбрать чат из выпадающего списка, но и **вручную вписать нужный `peer_id`**, если API ВКонтакте отказывается его показывать.
